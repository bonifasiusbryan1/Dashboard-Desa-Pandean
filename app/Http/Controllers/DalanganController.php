...
class DalanganController extends Controller
{
    public function dalanganData()
    {
        ...
        $mataPencaharianData = MataPencaharian::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $usiaData = Usia::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $kualitasBayiData = KualitasBayi::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $kualitasIbuHamilData = KualitasIbuHamil::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $statusBalitaData = StatusBalita::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $imunisasiData = Imunisasi::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $rasioGuruMuridData = RasioGuruMurid::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $statusPendidikanData = WajibBelajar::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $agamaData = Agama::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $tingkatPendidikanData = TingkatPendidikan::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $kebutuhanAirData = KebutuhanAir::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        $asetTanahData = AsetTanah::where('id_dusun', 5)
            ->where('id_tahun', 1)
            ->get();
        ...
        return view('dashboard.dalangan', [
    ...
    public function dalanganData_2()
    {
        $pendapatanData = PendapatanPerkapita::with('tahun', 'dusun', 'jenisSektor')
        ->where('id_dusun', 5)
        ->where('id_tahun', 1)
        ...
        return view('dashboard.dalangan_2', [
            'pendapatanChartData' => $pendapatanChartData,
            'sectors' => $sectors,
            'totalPekerja' => $totalPekerja,
            'pekerjaanChartData' => $pekerjaanChartData,
    ...
    public function formPendapatan(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('pendapatanPerkapita'));
    }
    ...
    public function formMataPencaharian(Request $request)
    {
        ...
        $perorangan = MataPencaharian::where('id_dusun', $id_dusun)
            ->where('id_tahun', $id_tahun)
            ->pluck('perorangan', 'id_sektor')
            ->toArray();   

        $usaha = MataPencaharian::where('id_dusun', $id_dusun)
            ->where('id_tahun', $id_tahun)
            ->pluck('usaha', 'id_sektor')
            ->toArray();   

        $buruh = MataPencaharian::where('id_dusun', $id_dusun)
            ->where('id_tahun', $id_tahun)
            ->pluck('buruh', 'id_sektor')
            ->toArray();   

        $jumlah = MataPencaharian::where('id_dusun', $id_dusun)
            ->where('id_tahun', $id_tahun)
            ->pluck('jumlah', 'id_sektor')
            ->toArray();   

        return view('inputdata.dalangan', compact('perorangan', 'usaha', 'buruh', 'jumlah'));
    }
    ...
    public function storeMataPencaharian(Request $request)
    {
        ...
        $fields = [
            'peroranganpertanian',
            'peroranganpeternakan',
            'peroranganjasaperdagangan',
            'peroranganindustri',
            'usahapertanian',
            'usahapeternakan',
            ...
            'buruhpertanian',
            'buruhpeternakan',
            'buruhjasaperdagangan',
            'buruhindustri',
            'jumlahpertanian',
            'jumlahpeternakan',
            'jumlahjasaperdagangan',
            'jumlahindustri',
        ];

        $sanitized = [];
        foreach ($fields as $f) {
            // Hapus titik sebagai pemisah ribuan dan ambil hanya angka
            $rawValue = $request->input($f, '');
            $cleanValue = str_replace(['.', ','], '', $rawValue);
            $sanitized[$f] = substr(preg_replace('/\D/', '', $cleanValue), 0, 10);
        }

        $rules = [];
        foreach ($fields as $f) {
            $rules[$f] = ['required', 'regex:/^\d{1,10}$/'];
        }
        ...
        if ($validator->fails()) {
            return back()
                ->withErrors(['mata_pencaharian' => 'Gagal! Mohon lengkapi data terlebih dahulu.'])
                ->withInput();
        }

        $data = [
            ['id_sektor' => 2, 'perorangan' => $sanitized['peroranganpertanian'],       'usaha' => $sanitized['usahapertanian'],       'buruh' => $sanitized['buruhpertanian'],       'jumlah' => $sanitized['jumlahpertanian']],
            ['id_sektor' => 1, 'perorangan' => $sanitized['peroranganpeternakan'],      'usaha' => $sanitized['usahapeternakan'],      'buruh' => $sanitized['buruhpeternakan'],      'jumlah' => $sanitized['jumlahpeternakan']],
            ['id_sektor' => 3, 'perorangan' => $sanitized['peroranganjasaperdagangan'], 'usaha' => $sanitized['usahajasaperdagangan'], 'buruh' => $sanitized['buruhjasaperdagangan'], 'jumlah' => $sanitized['jumlahjasaperdagangan']],
            ['id_sektor' => 4, 'perorangan' => $sanitized['peroranganindustri'],        'usaha' => $sanitized['usahaindustri'],        'buruh' => $sanitized['buruhindustri'],        'jumlah' => $sanitized['jumlahindustri']],
        ];

        foreach ($data as $row) {
            MataPencaharian::updateOrCreate(
                ['id_dusun' => $id_dusun, 'id_tahun' => $id_tahun, 'id_sektor' => $row['id_sektor']],
                ['perorangan' => $row['perorangan'], 'usaha' => $row['usaha'], 'buruh' => $row['buruh'], 'jumlah' => $row['jumlah']]
            );
        }

        return redirect()->back()->with('success_mata_pencaharian', 'Success! Data berhasil disimpan.');
    }
    ...
    public function formPekerjaan(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('pekerjaan', 'year'));
    }
    ...
    public function formUsia(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('usiaData', 'year'));
    }
    ...
    public function formKualitasBayi(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('kualitasBayiData', 'year'));
    }
    ...
    public function formKualitasIbuHamil(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('kualitasIbuHamilData', 'year'));
    }
    ...
    public function formStatusBalita(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('statusBalitaData', 'year'));
    }
    ...
    public function formImunisasi(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('imunisasiData', 'year'));
    }
    ...
    public function formRasioGuruMurid(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('rasioGuruMurid', 'year'));
    }
    ...
    public function formStatusPendidikan(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('statusPendidikan', 'year'));
    }
    ...
    public function formAgama(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('agamaData', 'year'));
    }
    ...
    public function formTingkatPendidikan(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('tingkatPendidikanData', 'year'));
    }
    ...
    public function formKebutuhanAir(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('kebutuhanAirData', 'year'));
    }
    ...
    public function formAsetTanah(Request $request)
    {
        ...
        return view('inputdata.dalangan', compact('asetTanahData', 'year'));
    }
